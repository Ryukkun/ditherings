...
public class Main {
    public static void main( String[] args ) throws IOException {
        ...
        int[][] colors = {{0,0,0},{255,0,0},{255,255,0},{0,255,0},{0,255,255},{255,0,255},{0,0,255},{255,255,255}};

        BufferedImage img = ImageIO.read( new File( "D:/Desktop/GIt Hub/ditherings/target.jpg" ) );

        int cols = colors.length;
        int yy = img.getHeight(), xx = img.getWidth();
        int[] target = new int[xx * yy];
        img.getRGB(0, 0, xx, yy, target, 0, xx);
        ...
        for (int y = 0; y < yy; y++){
            ...
            for (int x = 0; x < xx; x++){
                ...
                for (i = 0; i < cols; i++) {
                    ...
                    for (ii = 0; ii < 3; ii++) {
                        n = colors[i][ii] - res[ii];
                        total += n * n;
                    }
                    if (total < last_total || last_total == -1) {
                        last_total = total;
                        color_index = i;
                    }
                }
                ...
                for (ii = 0; ii < 3; ii++){
                    if (0 < res[ii]){
                        diffe[ii] = res[ii] - colors[color_index][ii];
                    }else{
                        diffe[ii] = res[ii] + colors[color_index][ii];
                    }
                }
        ...
        ImageIO.write(img, "png", new File( "D:/Desktop/GIt Hub/ditherings/test.jpg" ) );
